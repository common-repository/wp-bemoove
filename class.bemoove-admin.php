...
class BeMoOve_Admin_Class {
    ...
    function add_pages() {
        ...
        if ($this->getUserAccountInfo()->hasAccount()) {
			...
			if($this->getUserAccountInfo()->getAccountActivate() == 'F'){
				add_menu_page('BeMoOve','BeMoOve', 'level_8', 'BeMoOve_wait', array($this, 'BeMoOve_Wait_Page'), '', NULL);
			}else{
...
function BeMoOve_Contact_Page(){
	...
	wp_enqueue_script('bemoovejs_JQ', $jsRoot . '/jquery-1.11.1.min.js', array());
	wp_enqueue_script('bemoovejs_ad', $jsRoot . '/admin.js', array());
    }
	
	//Activate=Fの状態
	function BeMoOve_Wait_Page() {
		$userAccountInfo = UserAccountInfo::createInstance($this->getUserAccountInfo()->getAccountId(), $this->getUserAccountInfo()->getAccountApiprekey(), $this->getUserAccountInfo()->getAccountUsername(), $this->getUserAccountInfo()->getAccountComname(), $this->getUserAccountInfo()->getAccountUsermail(), $this->getUserAccountInfo()->getAccountUsertel());
	...
	if ($accountdata && $accountdata[getAccount][item][activate] == 'T') {
		$userAccountInfo = UserAccountInfo::createInstance(
			$this->getUserAccountInfo()->getAccountId()
			, $this->getUserAccountInfo()->getAccountApiprekey()
			, $this->getUserAccountInfo()->getAccountUsername()
			, $this->getUserAccountInfo()->getAccountComname()
			, $this->getUserAccountInfo()->getAccountUsermail()
			, $this->getUserAccountInfo()->getAccountUsertel()
			, 'T'
		);
					
		$userAccountInfo->save();
		
		$location = admin_url() . 'admin.php?page=BeMoOve_movies_list';
		die("<script type=\"text/javascript\">(function() { location.href = \"{$location}\"; })();</script>");
...
<div class="wrap welcome_area">
    <h2>ご利用登録申請完了</h2>
    <div class="info">
        WP-BemoovePluginのご利用登録が申請されました。登録手続きのため、運営会社よりご連絡を取らせていただきます。<br>
		登録完了まで今しばらくお待ちください。
    </div>
	...
	※ご登録情報<br>
	<table>
	<tr><td>account_id</td><td>:</td><td><?php print($this->getUserAccountInfo()->getAccountId());?></td></tr>
	<tr><td>お名前</td><td>:</td><td><?php print($this->getUserAccountInfo()->getAccountUsername());?></td></tr>
	<tr><td>会社名</td><td>:</td><td><?php print($this->getUserAccountInfo()->getAccountComname());?></td></tr>
	<tr><td>メールアドレス</td><td>:</td><td><?php print($this->getUserAccountInfo()->getAccountUsermail());?></td></tr>
	<tr><td>電話番号</td><td>:</td><td><?php print($this->getUserAccountInfo()->getAccountUsertel());?></td></tr>
	</table>
	</div>
    ...
    <div class="account_setting_detail">
        <h3>■ アカウント削除</h3>
        <form action="admin.php?page=BeMoOve_wait&deleted=1" method="post">
...
<?php
    ...
    function BeMoOve_Welcome_Page() {
		...
		if (empty($_POST['account_username'])) {
			$errOnRegisterAccount .= 'お名前が入力されていません。 <br />'; 
		}
		if (empty($_POST['account_comname'])) {
			$errOnRegisterAccount .= '会社名が入力されていません。 <br />'; 
		}
		if (empty($_POST['account_usermail'])) {
			$errOnRegisterAccount .= 'メールアドレスが入力されていません。 <br />'; 
		}
		if (empty($_POST['account_usertel'])) {
			$errOnRegisterAccount .= '電話番号が入力されていません。 <br />'; 
		}
        ...
        if ($isRestartAccount) {
			...
			//お名前、会社名、メールアドレス、電話番号
			if (empty($_POST['account_username'])) {
				$errOnRestartAccount .= 'お名前が入力されていません。 <br />'; 
			}
			if (empty($_POST['account_comname'])) {
				$errOnRestartAccount .= '会社名が入力されていません。 <br />'; 
			}
			if (empty($_POST['account_usermail'])) {
				$errOnRestartAccount .= 'メールアドレスが入力されていません。 <br />'; 
			}
			if (empty($_POST['account_usertel'])) {
				$errOnRestartAccount .= '電話番号が入力されていません。 <br />'; 
			}
            ...
            if (empty($errOnRestartAccount)) {
                $userAccountInfo = UserAccountInfo::createInstance($accountId, $accountApiprekey, $_POST['account_username'], $_POST['account_comname'], $_POST['account_usermail'], $_POST['account_usertel']);
			...
			if(empty($errOnRegisterAccount)){
				
				// アカウント新規作成時
				$accountdata = BeHLSApiClient::addAcount($_POST['account_username'],$_POST['account_comname'],$_POST['account_usermail'],$_POST['account_usertel']);
				...
				if ($accountdata && $accountdata[addAccount][item][activate] == 'T') {
					...
					$userAccountInfo = UserAccountInfo::createInstance(
						...
						, $_POST['account_comname']
						, $_POST['account_usermail']
						, $_POST['account_usertel']
						, 'F'
					);
				...
				echo "<script>window.location.href='?page=BeMoOve_welcome&error=2';</script>";
...
<?php
            ...
            // アカウント新規登録ページの場合 
                // Waitページへリダイレクト
                $location = admin_url() . 'admin.php?page=BeMoOve_wait';
                die("<script type=\"text/javascript\">(function() { location.href = \"{$location}\"; })();</script>");
...
<div class="wrap welcome_area">
    ...
    <form action="" method="post">
        <table class="form-table">
            ...
            <tr>
                <th><label for="account_username">お名前</label></th>
                <td><input name="account_username" type="text" id="account_username" class="regular-text" value="<?php print($_POST['account_username']); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_comname">会社名</label></th>
                <td><input name="account_comname" type="text" id="account_comname" class="regular-text" value="<?php print($_POST['account_comname']); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_usermail">メールアドレス</label></th>
                <td><input name="account_usermail" type="text" id="account_usermail" class="regular-text" value="<?php print($_POST['account_usermail']); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_usertel">電話番号</label></th>
                <td><input name="account_usertel" type="text" id="account_usertel" class="regular-text" value="<?php print($_POST['account_usertel']); ?>" /></td>
            </tr>
...
<div class="wrap welcome_area">
    ...
    <div class="info">
    	...
    	if($_GET['error']==1){ 
			...
			echo "<font color='red'>必要項目が入力されていません。すべてご入力の上ご登録ください。</font><br>"; 
		}
    	?> 
        WP-BemoovePluginは、<a target="blank" href="http://www.bemoove.jp/"> ビムーブ株式会社 </a>  が提供するWordPress上で動画を簡単に配信することができる<br /> 
        プラグインです。<br /> 
		当プラグインのご利用は法人のお客様に限ります。個人名義のお客様はご利用になれませんので、予めご了承ください。<br /> 
    ...
    <div style="max-width: 750px;">
        ...
        <div class="form_area">
            <form action="" method="post">
				...
				※下記内容をご入力ください。
				<table>
				<tr><td>お名前</td><td>:</td><td><input name="account_username" type="text" id="inputtext3" class="regular-text" value="" /></td></tr>
				<tr><td>会社名</td><td>:</td><td><input name="account_comname" type="text" id="inputtext3" class="regular-text" value="" /></td></tr>
				<tr><td>メールアドレス</td><td>:</td><td><input name="account_usermail" type="text" id="inputtext3" class="regular-text" value="" /></td></tr>
				<tr><td>電話番号</td><td>:</td><td><input name="account_usertel" type="text" id="inputtext3" class="regular-text" value="" /></td></tr>
				</table>
				</br>
...
<?php
    ...
    function BeMoOve_Admin_Page() {
        ...
        if ($_POST['edit_account'] == '1') {
            ...
            $newAccountUsername = $opt[UserAccountInfo::ACCOUNT_USER_NAME];
            $newAccountComname = $opt[UserAccountInfo::ACCOUNT_COM_NAME];
            $newAccountUsermail = $opt[UserAccountInfo::ACCOUNT_USER_MAIL];
            $newAccountUsertel = $opt[UserAccountInfo::ACCOUNT_USER_TEL];
            ...
            if (empty($newAccountUsername)) {
                $editErrMsg .= 'お名前は必須入力項目です。<br />'; 
            }
            if (empty($newAccountComname)) {
                $editErrMsg .= '会社名は必須入力項目です。<br />'; 
            }
            if (empty($newAccountUsermail)) {
                $editErrMsg .= 'メールアドレスは必須入力項目です。<br />'; 
            }
            if (empty($newAccountUsertel)) {
                $editErrMsg .= '電話番号は必須入力項目です。<br />'; 
            }
            ...
            if (empty($editErrMsg)) {
				...
				$userAccountInfo = UserAccountInfo::createInstance($newAccountId, $newAccountApiprekey);
				$apiClient = new BeHLSApiClient($userAccountInfo);
                $accountdata = $apiClient->getAccount($bmSub_wpdomain);  //<--------------- V 1.4用の更新プログラム
				
				if (empty($accountdata) || $accountdata[getAccount][item][activate]!= 'T') {
					$editErrMsg .= '入力されたアカウント情報が正しくありません。[Activate:Error]<br />'; 
				}
...
<div class="wrap account_setting_content">
    ...
    <div class="account_setting_detail">
        ...
        <form action="" method="post">
            ...
            <tr>
                <th><label for="account_username">お名前</label></th>
                <td><input name="<?php print(UserAccountInfo::OPTION_KEY . '[' . UserAccountInfo::ACCOUNT_USER_NAME . ']') ?>" type="text" id="account_username" class="regular-text" value="<?php print($this->getUserAccountInfo()->getAccountUsername()); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_comname">会社名</label></th>
                <td><input name="<?php print(UserAccountInfo::OPTION_KEY . '[' . UserAccountInfo::ACCOUNT_COM_NAME . ']') ?>" type="text" id="account_comname" class="regular-text" value="<?php print($this->getUserAccountInfo()->getAccountComname()); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_usermail">メールアドレス</label></th>
                <td><input name="<?php print(UserAccountInfo::OPTION_KEY . '[' . UserAccountInfo::ACCOUNT_USER_MAIL . ']') ?>" type="text" id="account_usermail" class="regular-text" value="<?php print($this->getUserAccountInfo()->getAccountUsermail()); ?>" /></td>
            </tr>
			
            <tr>
                <th><label for="account_usertel">電話番号</label></th>
                <td><input name="<?php print(UserAccountInfo::OPTION_KEY . '[' . UserAccountInfo::ACCOUNT_USER_TEL . ']') ?>" type="text" id="account_usertel" class="regular-text" value="<?php print($this->getUserAccountInfo()->getAccountUsertel()); ?>" /></td>
            </tr>
...
<div class="wrap">
    ...
    <form action="admin.php?page=BeMoOve_new&m=file_upload" method="post">
        <table class="form-table">
            <tr valign="top">
                <th scope="row"><label for="inputtext">アップロード動画名</label></th> 
                <td><input name="movie_name" type="text" id="inputtext" pattern="^[\w\-]+$" value="" placeholder="半角英数で入力してください(80文字まで) " class="regular-text" maxlength="80"/></td> 
            </tr>
        </table>
